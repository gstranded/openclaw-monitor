...
为减少后续从文档切到代码时的信息损耗，建议正式开工前逐项确认以下检查项。

### 22.1 页面与路由

- [ ] 已存在可承载页面的前端应用骨架
- [ ] 已确定 Agent 详情页路由形态（页内状态或独立文件 URL）
- [ ] 已约定空态 / 错误态 / 骨架屏的基础组件复用方式

### 22.2 数据与接口

- [ ] Agent 详情接口字段与本文档一致或已明确差异
- [ ] Markdown 文件列表接口能返回 `editable` / `exists` / `lastModifiedAt`
- [ ] 文件内容接口能返回 `version` 以支持冲突判断
- [ ] 保存接口能区分 `forbidden` / `conflict` / `workspace_readonly`

### 22.3 交互与边界

- [ ] 未保存切换确认的触发时机已统一
- [ ] 只读文件的原因文案来自后端或有统一映射表
- [ ] 文件缺失时保留列表位的行为已与后端对齐
- [ ] 保存失败后保留本地编辑内容的策略已纳入实现

### 22.4 验证准备

- [ ] mock 数据至少覆盖 `editable` / `readonly` / `missing` / `conflict`
- [ ] 已确定最小可演示文件（建议 `TASK.md`）
- [ ] QA 已有可复用的场景清单，不必等真实接口全量完成
...
- Issue #25 已核对，当前阻塞不在需求描述，而在仓库尚无可承载页面实现的前端工程骨架
